...
# Если в контейнер были переданы аргументы (command в docker-compose),
# запускаем их вместо стандартного приложения.
if [ "$#" -gt 0 ]; then
    echo "5. Starting custom command: $@"
    exec "$@"
else
    echo "5. Starting main application (python main.py)..."
    exec python main.py
fi